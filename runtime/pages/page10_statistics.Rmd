...
Use paired t-test instead (handy for questionnaires):
```{r}
...
observe({
  ...
  updateSelectizeInput(session, "depVar", choices = numeric_cols , selected = ifelse(input$diffData_stats, "diff_stepWidths.sd", "stepWidths.sd"))
  updateSelectizeInput(session, "indepInterVars", choices = numeric_cols, selected = ifelse(input$diffData_stats, "diff_VEQ.total", "VFD"))
  updateSelectizeInput(session, "indepVars", choices = numeric_cols)
})
...
output$ttest_text <- renderPrint({
  ...
  if (length(input$indepInterVars) > 0) {
    ...
    if (length(unique(data[[comparison_col]])) == 2) { # can only do t-test if 2 levels
      levels <- unique(data[[comparison_col]])

      before <- data[[dep_var]][data[[comparison_col]] == levels[1]]
      after <- data[[dep_var]][data[[comparison_col]] == levels[2]]
      ...
      # Interpretation of F-test (Homogeneity of variances)
      if (variance_test$p.value > 0.05) {
        cat("Interpretation: The p-value is", variance_test$p.value, 
            "> 0.05, so we assume the variances are equal (homogeneity of variances).\n")
      } else {
        cat("Interpretation: The p-value is", variance_test$p.value, 
            "< 0.05, indicating a violation of the homogeneity of variances assumption.\n")
      }
      ...
      if (t_test_result$p.value < 0.05) {
        cat("Interpretation: The p-value is", t_test_result$p.value, 
