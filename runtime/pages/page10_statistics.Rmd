...
checkboxInput("interaction", "Include Interaction", value = TRUE)
```
...
get_lmer <- reactive({
  ...
  indep_vars <- input$indepVars
  
  # Add interaction terms if selected
  if (input$interaction) {
    indep_vars_interaction <- paste(indep_vars, collapse = " * ")
  } else {
    indep_vars_interaction <- paste(indep_vars, collapse = " + ")
  }
  
  # Construct the formula text
  formula_text <- paste(dep_var, "~", indep_vars_interaction, "+ (1 | participant)")
