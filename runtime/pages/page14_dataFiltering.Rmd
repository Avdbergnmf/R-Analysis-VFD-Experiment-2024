...
numericInput("lof_k", "Number of Neighbors (k) for LOF", value = 15, min = 1)

# Input box for k value in LOF algorithm
numericInput("lof_k", "Max points (prevent crashing)", value = 1000, min = 1,step=100)
...
# Render the LOF score cumulative distribution plot with interactive points
output$lof_score_distribution_plotly <- renderPlotly({
  ...
  max_points <- 1000  # Limit to 1000 points for example
  if (nrow(plot_data) > max_points) {
    plot_data <- plot_data[seq(1, nrow(plot_data), length.out = max_points), ]
  }
