...
generate_questionnaire_plots <- function(questionnaireList, participant, baseSize = 10) {
  plots <- lapply(questionnaireList, function(questionnaire) {
    plot_questionnaire_data(questionnaire, participant, c(), baseSize = baseSize)
  })
  return(plots)
}
...
generate_variability_plots <- function(data, participant, varList, baseSize = 10) {
  plots <- lapply(varList, function(var) {
    plot_variability_data(data, participant, var[[1]], var[[2]], ylims = var[[3]], baseSize = baseSize)
  })
  return(plots)
}
...
observeEvent(input$generateReports, {
  ...
  for (i in seq_along(selected_participants)) {
    ...
    varList <- list(list("stepWidths","sd", c(0,0.1)),list("stepWidths","cv", c(0,0.3)), list("stepLengths","cv", c(0,0.3)))
    variability_plots <- generate_variability_plots(get_mu_dyn(), participant, varList, baseSize)
