...
generate_scatter_plot <- function(data, participant, baseSize = 10) {
  ...
  make_scatter_plot_steps(scatter_data, "VFD", "relHeelStrikes.pos.x", "relHeelStrikes.actual_pos.z", baseSize = baseSize)
}
...
observeEvent(input$generateReports, {
  ...
  for (i in seq_along(selected_participants)) {
    ...
    traj_plots_z <- generate_traj_plots(filteredParams(), participant, trials, axis_to_plot = "pos.z", doFilter, c(-0.6,0.6), baseSize)
    traj_plots_y <- generate_traj_plots(filteredParams(), participant, trials, axis_to_plot = "pos.y", doFilter, c(0.4,0.7), baseSize)
    traj_plots_x <- generate_traj_plots(filteredParams(), participant, trials, axis_to_plot = "pos.x", doFilter, c(-0.4,0.4), baseSize)
    
    pie_charts <- generate_pie_charts(allGaitParams, participant, trials, baseSize)
    
    questionnaireList <- c("UserExperience")
    ...
    #eye_plots_x <- generate_2d_plots(filteredParams(), participant, trials, "eye", "hit_pos.x", baseSize, overrideLims = c(-1, 1))
    #eye_plots_z <- generate_2d_plots(filteredParams(), participant, trials, "eye", "hit_pos.z", baseSize, overrideLims = c(-1, 5))
